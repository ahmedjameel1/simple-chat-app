...
def chat(request, pk):
    if request.method == 'POST':
        ...
        message = Message.objects.create(
            body=body, sender=request.user, recipient=User.objects.get(id=pk))
        message.save()
        ...
        recieved_messages = Message.objects.filter(
            sender=recipient, recipient=user).order_by('created')
        sent_messages = Message.objects.filter(
            sender=user, recipient=recipient).order_by('created')
